...
const FilterModal = ({ appliedFilters, defaultFilters, onApply, onClose }) => {
    const [activeTab, setActiveTab] = useState('basic'); // 'basic' | 'advance'
    const [draft, setDraft] = useState(() => ({ ...appliedFilters }));

    const updateDraft = (field, value) => setDraft((prev) => ({ ...prev, [field]: value }));

    const pillSelector = (options, field, valueMap = (opt) => (opt === 'Any' ? '' : opt)) => (
            ...
            {options.map((opt) => {
                ...
                return (
                    <button
                        ...
                        className={`py-2 px-3 rounded-full font-semibold text-[12.5px] border cursor-pointer transition-all text-center ${
                            selected
    ...
    return (
        <div
            className="fixed inset-0 z-[100] flex items-end justify-center bg-black/60 backdrop-blur-xs animate-in fade-in duration-200 select-none max-w-[414px] mx-auto"
            onClick={onClose}
        >
            <div
                className="w-full bg-white rounded-t-[32px] p-5 pb-6 shadow-2xl overflow-y-auto max-h-[88vh] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden animate-in slide-in-from-bottom duration-250 border-t border-gray-100 flex flex-col justify-between"
                onClick={(e) => e.stopPropagation()}
            >
                <div>
                    {/* Top Switcher Pill Tabs */}
                    <div className="bg-[#F3EEFC] rounded-full p-0.5 flex items-center mb-3.5">
                        <button
                            type="button"
                    ...
                    {activeTab === 'advance' && (
                        <div className="mb-3 animate-in fade-in duration-200 flex flex-col gap-3.5">
                            <div>
                                <h4 className="font-semibold text-[12.5px] text-gray-900 mb-2">What are they looking for?</h4>
                                {pillSelector(RELATIONSHIP_GOAL_OPTIONS, 'relationshipGoal')}
                            </div>
                            <div>
                                <h4 className="font-semibold text-[12.5px] text-gray-900 mb-2">Religious beliefs</h4>
                                {pillSelector(RELIGION_OPTIONS, 'religion')}
                            </div>
                            <div>
                                <h4 className="font-semibold text-[12.5px] text-gray-900 mb-2">Education</h4>
                                {pillSelector(EDUCATION_OPTIONS, 'education')}
                            </div>
                            <div>
                                <h4 className="font-semibold text-[12.5px] text-gray-900 mb-2">Drinking</h4>
                                {pillSelector(HABIT_OPTIONS, 'drinkingStatus')}
                            </div>
                            <div>
                                <h4 className="font-semibold text-[12.5px] text-gray-900 mb-2">Smoking</h4>
                ...
                <div className="flex gap-2.5 mt-2 mb-1 shrink-0">
                    <button
                        type="button"
                        onClick={() => setDraft({ ...defaultFilters })}
                        className="flex-1 h-[44px] rounded-full bg-white border border-[#D8C6FE] text-[#703DE2] font-extrabold text-[14.5px] cursor-pointer tracking-wide active:scale-[0.98] transition-all"
                    >
                        Reset
                    </button>
                    <button
                        ...
                        className="flex-1 h-[44px] rounded-full bg-[#703DE2] hover:bg-[#602ec3] text-white font-extrabold text-[14.5px] shadow-md shadow-purple-200 cursor-pointer border-0 tracking-wide active:scale-[0.98] transition-all"
                    >
                        Apply
                    </button>
                </div>
